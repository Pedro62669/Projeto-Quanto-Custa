...
class PricingEngineTest extends TestCase
{
    ...
    #[Test]
    public function a_mailer_soma_os_paineis_do_die_cut(): void
    {
        // Mailer 300×200×150 (l×a×p), espessura 0:
        //   rolo   = 2×200 + 20 de trava                        =    420
        //   orelha = min(200, 150)                              =    150
        //   fundo + tampa   = 2 × (300 × 150)                   =  90.000
        //   frente+trás+lingueta = 3 × (300 × 200)              = 180.000
        //   laterais roladas     = 2 × (420 × 150)              = 126.000
        //   orelhas              = 4 × 150²                     =  90.000
        //   abas da tampa        = 2 × (200 × 150)              =  60.000
        //   total = 546.000 mm² = 0,546 m²
        $result = $this->engine->calculate($this->input([
            'boxModel' => BoxModel::Mailer,
        ]));

        $this->assertEqualsWithDelta(0.546, $result->areaM2PerUnit, 0.000001);
    }

    #[Test]
    public function a_mailer_consome_mais_que_um_rsc_do_mesmo_tamanho(): void
    {
        // A diferença é real e é o ponto do modelo: a mailer tem parede lateral
        // DUPLA (o rolo), tampa inteira e lingueta. Se algum dia esta asserção
        // inverter, é sinal de que o rolo deixou de ser contado dobrado.
        $mailer = $this->engine->calculate($this->input(['boxModel' => BoxModel::Mailer]));
        $rsc = $this->engine->calculate($this->input(['boxModel' => BoxModel::Rsc]));

        $this->assertGreaterThan($rsc->areaM2PerUnit, $mailer->areaM2PerUnit);
    }

    #[Test]
    public function a_orelha_da_mailer_nao_passa_da_profundidade_da_caixa(): void
    {
        // Numa caixa alta e rasa a orelha é limitada pela profundidade: sem o
        // min(h, d) ela ultrapassaria a parede oposta — material cobrado por
        // uma aba que não caberia dentro da própria caixa.
        //
        // Mailer 200×300×120, espessura 0:
        //   rolo = 2×300 + 20 = 620 ; orelha = min(300, 120) = 120
        //   2×(200×120) + 3×(200×300) + 2×(620×120) + 4×120² + 2×(300×120)
        //   = 48.000 + 180.000 + 148.800 + 57.600 + 72.000 = 506.400 mm²
        $result = $this->engine->calculate($this->input([
            'boxModel' => BoxModel::Mailer,
            'widthMm' => 200, 'heightMm' => 300, 'depthMm' => 120,
        ]));

        $this->assertEqualsWithDelta(0.5064, $result->areaM2PerUnit, 0.000001);
    }

    #[Test]
    public function a_lateral_rolada_faz_a_altura_pesar_mais_que_num_rsc(): void
    {
        // Cada milímetro de altura entra DUAS vezes na lateral da mailer (sobe
        // e desce) contra uma no RSC. Crescer a altura precisa portanto encarecer
        // a mailer mais do que o RSC — é a assinatura de custo do modelo.
        $delta = function (BoxModel $model): float {
            $baixa = $this->engine->calculate($this->input([
                'boxModel' => $model, 'widthMm' => 300, 'heightMm' => 100, 'depthMm' => 150,
            ]));
            $alta = $this->engine->calculate($this->input([
                'boxModel' => $model, 'widthMm' => 300, 'heightMm' => 300, 'depthMm' => 150,
            ]));

            return $alta->areaM2PerUnit - $baixa->areaM2PerUnit;
        };

        $this->assertGreaterThan($delta(BoxModel::Rsc), $delta(BoxModel::Mailer));
    }

    #[Test]
    public function a_mailer_nao_tem_tampa_separada(): void
    {
        // A tampa é ARTICULADA: sai da mesma chapa e não é peça avulsa. Expor
        // campos de tampa aqui faria o usuário dimensionar algo inexistente.
